...
const EventDetailsPage = () => {
  ...
  const likeMoment = (moment) =>
    act('post', `/api/events/${id}/moments/${moment.id}/like`).then((r) => {
      if (!r) return;
      setMoments((prev) => prev.map((x) => (x.id === moment.id ? normalizeMoment(r.moment, meId) : x)));
    });
        ...
        <div className="space-y-6">
          ...
          <div className="card p-5">
            <SquadList
              ...
              onMemberClick={(uid) => navigate(`/people/${uid}`)}
              onChat={null}
            />
          </div>
